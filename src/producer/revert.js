...
async function unpublishUsingLogs (logFileName) {
  ...
  if (doesFileExistInGivenDirectory(logFileName, logsDir)) {
            ...
            logs.locale.forEach((loc, locIndex) => {
              logs.entries[loc].forEach(({ publish_details, uid, locale, content_type }, entryIndex) => {
                ...
                if (publishDetailsForThisEnvironment.length > 0) {
                  ...
                  if (bulkUnpublishSet.length === 10) {
                    unpublishQueue.Enqueue({ entries: bulkUnpublishSet, environments: [environment.name], locale: loc, Type: 'entry' })
                    bulkUnpublishSet = []
                  }
                }
...
async function formatLogData (data) {
  ...
  if (type === 'entry') {
    formattedLogs.entries = filterPublishDetails(formattedLogs.entries, formattedLogs.environments.map(({ uid }) => { return uid }), formattedLogs.locale)
  } else {
    formattedLogs.assets = filterPublishDetails(formattedLogs.assets, formattedLogs.environments.map(({ uid }) => { return uid }))
  }
...
async function getEnvironmentUids (environments) {
  try {
    ...
    const filteredEnvironments = allEnvironments.environments.filter((environment) => environments.indexOf(environment.name) !== -1).map(({ name, uid }) => { return { name, uid } })
