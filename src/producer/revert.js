            ...
            logs.environments.forEach((environment) => {
                ...
                switch (logs.type) {
                    case 'entry':
                        ...
                        logs.entries.forEach(({ publish_details, uid, locale, content_type }, entryIndex) => {
                            ...
                            if (publishDetailsForThisEnvironment.length > 0) {
                                ...
                                publishDetailsForThisEnvironment.forEach((publish_detail, publish_detail_index) => {
                                    ...
                                    if (bulkPublishSet.length < 10) {
                                        bulkPublishSet.push({
                                            ...
                                            locale: publish_detail.locale,
                                            content_type
                                        })
                                    }

                                    if (bulkPublishSet.length === 10) {
                                        revert_queue.Enqueue({ entries: bulkPublishSet, environments: [environment.name], locale: locale, Type: 'entry' });
                                        bulkPublishSet = [];
                                    }
                                ...
                                if (bulkUnpublishSet.length === 10) {
                                    unpublish_queue.Enqueue({ entries: bulkUnpublishSet, environments: [environment.name], locale: locale, Type: 'entry' });
                                    bulkUnpublishSet = [];
                                }

                            }

                            if (entryIndex === logs.entries.length - 1) {
...
async function formatLogData(data) {
        ...
        case 'entry':
            ...
            for (let i = 0; i < data.length; i++) {
                if (data[i].message.options.entries) {
                    ...
                    formatted_logs['entries'] = formatted_logs['entries'].concat(data[i].message.options.entries);
                } else {
                    // for handling logs created by publishing in a regular way
                    formatted_logs['entries'].push({
                ...
                if (!formatted_logs['locale']) formatted_logs['locale'] = data[i].message.options.locale;
    ...
    if (type === 'entry') {
        ...
        formatted_logs.assets = filterPublishDetails(formatted_logs.assets, formatted_logs.environments.map(({ uid }) => { return uid }), formatted_logs['locale']);
    }
...
function filterPublishDetails(elements, environments, locale) {
    for (i = 0; i < elements.length; i++) {
        elements[i].publish_details = elements[i].publish_details.filter((element) => environments.indexOf(element.environment) !== -1 && element.locale === locale);
    }
    return elements;
}
